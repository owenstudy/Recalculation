...
-------check the product level recalculation result
 ...
 select product_id,  sum(decode(a.passed,'Y',1,0)) Pass_Amt,count(*) total_amt from dc_contract_product_cashvalue a group by a.product_id
...
-----------total pass rate
 select   pass_amt, total_amt, round(pass_amt/total_amt,2) passrate  from (
  select  sum(decode(a.passed,'Y',1,0)) Pass_Amt,count(*) total_amt from dc_contract_product_cashvalue a
 );
